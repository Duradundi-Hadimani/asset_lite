frappe.pages['asset-map'].on_page_load = function(wrapper) {
...
/*
	initialize_map() {
    // Initialize map centered more precisely in Saudi Arabia with zoom controls and limits
    this.map = L.map('asset_map', {
        zoomControl: true,
        minZoom: 5,
        maxZoom: 10,
        zoomSnap: 0.25  // Enable fractional zoom for smooth zooming
    }).setView([24.8, 45.5], 6);  // More centered and slightly less zoomed
    ...
    // Saudi Arabia polygon GeoJSON
    const saudiGeoJSON = { 
        "type": "Feature",
        "geometry": {
            "type": "Polygon",
            "coordinates": [[
                [34.5, 16.0], [55.7, 16.0], [55.7, 32.2], [34.5, 32.2], [34.5, 16.0]
            ]]
        }
    };

    // World polygon for outside mask
    const world = {
        "type": "Polygon",
        "coordinates": [[
    ...
    // Mask outside Saudi Arabia polygon (dark, almost opaque)
    L.geoJSON({
        ...
        "features": [
            {
                ...
                "geometry": {
                    ...
                    "coordinates": [
                        world.coordinates[0],
                        saudiGeoJSON.geometry.coordinates[0].slice().reverse()
                    ]
                }
            }
        ]
    ...
    // Draw Saudi Arabia polygon on top
    L.geoJSON(saudiGeoJSON, { 
        style: { color: '#ffcc00', weight: 2, fillColor: '#ffe699', fillOpacity: 0.3 } 
    }).addTo(this.map);

    // Define bounds for Saudi Arabia
    const saudiBounds = [[16.0, 34.5], [32.2, 55.7]];

    // Fit map bounds with very small padding to closely fit
    this.map.fitBounds(saudiBounds, { padding: [5, 5] });

    // Zoom out slightly if desired to make view a bit smaller and centered
    let currentZoom = this.map.getZoom();
    if(currentZoom > 5) {
        this.map.setZoom(currentZoom - 1);
    }

    // Fetch and render hospital/location data
